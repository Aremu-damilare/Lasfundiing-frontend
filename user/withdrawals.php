...
<div class="topbar" style="display: none;">
  ...
  <script>
    ...
    var currentAmount = document.getElementById("amount");
    var foundActiveOrder = false;  // Flag to track if any active order is found

    for (var i = 0; i < OrderList.length; i++) {
        var order = OrderList[i];
        console.log("Xxxxxxxxxxxxxxxxxxxxxx", order);

        if (order.active === true && order.status === "success" && order.stage === "active") {
            console.log("Matching Object:", order);
            currentOrderProfit.innerHTML = `$${order.profit}`;
            currentAmount.max = order.profit;
            currentAmount.value = order.profit;
            // Set the flag to true since an active order is found
            foundActiveOrder = true;
            break;  // Exit the loop since an active order is found
        }
    }

    // Check the flag to determine whether to remove the button
    if (!foundActiveOrder) {
        console.log("No active account");
        var withdrawalButton = document.getElementById('creatWithdrawalBtn');
        if (withdrawalButton) {
            withdrawalButton.remove();
        } else {
...
async function sortTableByDateRange() {
  try {
    ...
    setElementDisplayByClassName('mini-loader-container', 'block')
    const userWithdrawalsList = await userWithdrawalsSortDateRange(accessToken,  EndTimeSortValue=`${EndTimeSortValue}`, StartTimeSortValue=`${StartTimeSortValue}`, Order="newest" );
    setElementDisplayByClassName('mini-loader-container', 'none');
    console.log("userWithdrawalsList", userWithdrawalsList);
    appendRowToTable(userWithdrawalsList);
  } catch (error) {      
