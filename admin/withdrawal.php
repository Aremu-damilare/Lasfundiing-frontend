...
<head>
  <meta charset="utf-8">
  <title>Withdrawals detail - lasfunding</title>
...
<div class="topbar">
      ...
      <nav role="navigation" class="rl_navbar1_menu is-page-height-tablet w-nav-menu">              
        ...
        <div class="right-navbar-wrapper">
          ...
          <div class="right-navbar-link"><img src="../images/icons/admin/ArrowBendUpLeft.svg" loading="lazy" alt="" class="sign-out-icon">
            <a href="./withdrawals.php" style="color: #E04800;font-style: oblique;"  class="rl_navbar1_link w-nav-link">Withdrawals</a>
          </div>
    ...
    <div class="main-section-real">
      <div class="gradient-header">      
        <div style="color: #E04800;font-style: oblique;"  class="text-block-57">
          <span>Withdrawals detail #<span id="id"></span> </span>        
        </div>        
      </div>
      ...
      <div class="analyzer-section dashboard wf-section" style="display: block;">      
        ...
        <p class="info-block"><span class="info-label">Amount</span><span id="amount"></span></p>
  ...
  <script src="../backend/admin/getWithdrawals.js?<?php echo time(); ?>"></script>
  ...
  <script>
    ...
    async function main() {
      // Get the withdrawal ID from the query parameters
      const urlParams = new URLSearchParams(window.location.search);
      const withdrawalId = urlParams.get("id");
    
      // Check if withdrawalId exists and is an integer
      if (withdrawalId && !isNaN(withdrawalId) && Number.isInteger(Number(withdrawalId))) {
        const withdrawal = await getWithdrawal(baseUrl, accessToken, parseInt(withdrawalId));
    
        if (withdrawal) {
          withdrawalUpdateForm(withdrawal.id)
          console.log("withdrawal...", withdrawal)
          idElement.innerHTML = `${withdrawal.id}`;

          
          amount.innerHTML = `${withdrawal.amount}`;
          status.innerHTML = `${withdrawal.status}`;
           ...
           for (var i = 0; i < statusAdminOptions.options.length; i++) {
               if (statusAdminOptions.options[i].value === withdrawal.status) {
                 statusAdminOptions.options[i].selected = true;
                   break;
               }
           }

          lastUpdated.innerHTML = `${formatDate(withdrawal.updated_at)}`;
          dateCreated.innerHTML = `${formatDate(withdrawal.created_at)}`;
          

        } else {
          console.log('Failed to retrieve the withdrawal.');
        }
      } else {
        console.log('Invalid or missing withdrawal ID in the query parameter.');
      }
    }
  ...
  <script>
    async function withdrawalUpdateForm(withdrawalId) {
      ...
      form.addEventListener('submit', async (event) => {
          ...
          // Call the updateWithdrawal function and wait for the response
          const withdrawalUpdate = await updateWithdrawal(baseUrl, accessToken, parseInt(withdrawalId), formData);
          console.log("withdrawalUpdate", withdrawalUpdate);

          if (withdrawalUpdate !== false) {
            console.log("withdrawalUpdate not false", withdrawalUpdate);
            toastSuccessNotif("Success")
          } else {
            toastErrorNotif("Error")
            console.log("withdrawalUpdate false", withdrawalUpdate);
          }
      });
    }
  </script>
